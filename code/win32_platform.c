...
struct {
    // Platform non-pesific part
    int width, height;
    u32 *pixels;

    // Platform speific part
...
LRESULT CALLBACK
window_callback(HWND window, UINT message,
    ...
    switch (message) {
        ...
        case WM_SIZE: {
            ...
            render_buffer.pixels =
                VirtualAlloc(
                    0, sizeof(u32) * render_buffer.width * render_buffer.height,
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    ...
    HWND window =
        CreateWindowExA(0, window_class.lpszClassName,
                        "Breakout",
    ...
    while (running) {
        ...
        MSG message;
        while (PeekMessageA(&message, window, 0, 0,
                            PM_REMOVE)) {
